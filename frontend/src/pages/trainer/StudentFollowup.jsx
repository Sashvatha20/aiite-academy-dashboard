...
${form.remarks ? `\nRemarks:\n${form.remarks}` : ''}`.trim();
  ...
  const handleSyncStudentFollowupsSheet = async () => {
    try {
      ...
      if (res?.data?.success) {
        toast.success(
          res?.data?.message || `Student Followups sheet synced (${res?.data?.count || 0} rows)`
        );
      } else {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const payload = {
        ...
        no_of_interview_calls: parseInt(form.no_of_interview_calls, 10) || 0,
        no_of_rounds_cleared: parseInt(form.no_of_rounds_cleared, 10) || 0,
        interested: Boolean(form.interested),
        placed_status: form.placed_status,
      };
            ...
            <div style={S.row2}>
              <div style={S.field}>
                ...
                <input
                  ...
                  onChange={(e) => set('no_of_interview_calls', e.target.value)}
                />
              </div>
              <div style={S.field}>
                ...
                <input
                  ...
                  onChange={(e) => set('no_of_rounds_cleared', e.target.value)}
                />
              </div>
            </div>
        ...
        {tab === 1 && (
          <div>
            ...
            >
              ...
              <button type="button" style={{ ...S.btn, fontSize: 11 }} onClick={loadFollowups}>
                ↻ Refresh
              </button>
                  ...
                  <tbody>
                    {filtered.map((f) => (
                      <tr key={f.id}>
                        ...
                        <td style={S.td}>
                          <span style={S.badge(typeColor(f.followup_type))}>{f.followup_type}</span>
                        </td>
                        <td style={S.td}>
                          <span style={S.badge(callColor(f.call_status))}>
                            {f.call_status?.replace(/_/g, ' ')}
                          </span>
                        </td>
                        <td style={S.td}>{f.last_contact_date?.split('T')[0] || '—'}</td>
                        <td style={S.td}>
                          <span style={{ fontSize: 11, color: '#555' }}>{f.resume_status || '—'}</span>
                        </td>
                        <td style={S.td}>
                        ...
                        <td style={S.td}>
                          <span style={S.badge(placedColor(f.placed_status))}>
                            {f.placed_status?.replace(/_/g, ' ')}
                          </span>
                        </td>
                        <td style={S.td}>
                          <span style={S.badge(f.interested ? 'green' : 'red')}>
                            {f.interested ? 'Yes' : 'No'}
                          </span>
                        </td>
                        <td style={S.td}>
                          <button type="button" style={S.actBtn} onClick={() => handleEdit(f)}>
                            ✎ Edit
                          </button>
                        </td>
                      </tr>
