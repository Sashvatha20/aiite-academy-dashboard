...
export default function StudentFollowup() {
  ...
  const loadFollowups = useCallback(async () => {
    try {
      ...
      const list = Array.isArray(r.data) ? r.data : (r.data.followups || r.data.data || []);
      setFollowups([...list]);
    } catch (e) {
...
${form.remarks ? '\nRemarks:\n' + form.remarks : ''}`.trim();
  ...
  const handleSyncStudentFollowupsSheet = async () => {
    try {
      ...
      if (res?.data?.success) {
        toast.success(res?.data?.message || `Student Followups sheet synced (${res?.data?.count || 0} rows)`);
      } else {
        toast.error(res?.data?.error || 'Student Followups sheet sync failed');
      }
    } catch (err) {
      toast.error(err?.response?.data?.error || 'Failed to sync Student Followups sheet');
    } finally {
  ...
  const filtered = followups.filter(f => {
    const matchBatch = filterBatch ? String(f.batch_id) === String(filterBatch) : true;
      ...
      <div style={S.card}>
        ...
        <div style={S.tabs}>
          {['Log followup', 'View all'].map((t, i) => (
            <div key={i} style={{ ...S.tab, ...(tab === i ? S.tabA : {}) }} onClick={() => setTab(i)}>
              {t}
              {i === 1 && followups.length > 0 && (
                <span style={{ marginLeft: 6, fontSize: 10, background: '#E1F5EE', color: G, padding: '1px 6px', borderRadius: 8, fontWeight: 600 }}>
                  {followups.length}
                </span>
              )}
            </div>
          ...
          <div>
            {editId && (
              <div style={{ background: '#FAEEDA', border: '1px solid #FAC775', borderRadius: 8, padding: '8px 12px', fontSize: 11, color: '#854F0B', marginBottom: 10, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                ...
                <button type="button" style={S.actBtn} onClick={() => { setForm({ ...blank, last_contact_date: todayStr() }); setEditId(null); }}>✕ Cancel edit</button>
              </div>
            )}
            ...
            <div style={S.row2}>
              ...
              <div style={S.field}>
                ...
                <select style={S.select} value={form.student_id} onChange={e => set('student_id', e.target.value)}>
                  ...
                  {formStudents.map(s => (
                    <option key={s.id} value={s.id}>{s.candidate_name}{s.phone ? ` (${s.phone})` : ''}</option>
                  ))}
                </select>
              </div>
            </div>
            ...
            <div style={S.toggleRow}>
              <button type="button" style={S.toggle(form.interested)} onClick={() => set('interested', !form.interested)} aria-label="Toggle interest">
            ...
            <div style={S.field}>
              ...
              <textarea style={S.textarea} value={form.remarks} onChange={e => set('remarks', e.target.value)} placeholder="e.g. Student said resume will be ready by Monday..." />
            </div>
            ...
            <div style={S.btnRow}>
              ...
              <button type="button" style={S.btn} onClick={() => { setForm({ ...blank, last_contact_date: todayStr() }); setEditId(null); }}>
                Clear
              </button>
            </div>
          </div>
        ...
        {tab === 1 && (
          <div>
            <div style={{ display: 'flex', gap: 8, marginBottom: 12, flexWrap: 'wrap', alignItems: 'center' }}>
              ...
              <button type="button" style={{ ...S.btn, fontSize: 11 }} onClick={loadFollowups}>↻ Refresh</button>
              <span style={{ fontSize: 11, color: '#888' }}>{filtered.length} record{filtered.length !== 1 ? 's' : ''}</span>
            </div>
                  ...
                  <tbody>
                    {filtered.map(f => (
                      <tr key={f.id}>
                        ...
                        <td style={S.td}>
                          <button type="button" style={S.actBtn} onClick={() => handleEdit(f)}>✎ Edit</button>
                        </td>
                      </tr>
                    ))}
                  </tbody>
